...
/**
 * Core extension loaded via {@code -Dmaven.ext.class.path}, gated by {@code -Dsocket.task=socket-facts}
...
@Named("socket-facts")
@Singleton
public class SocketFactsLifecycleParticipant extends AbstractMavenLifecycleParticipant {

  private static final Logger LOG = LoggerFactory.getLogger("socket-facts");
  ...
  @Inject
  public SocketFactsLifecycleParticipant(
  ...
  public void afterSessionEnd(MavenSession session) throws MavenExecutionException {
    if (!"socket-facts".equals(normalize(opt(session, "socket.task")))) {
      return;
    }
