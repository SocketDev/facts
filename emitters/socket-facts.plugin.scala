 ...
 * with no way to silence it). The TS assembler (src/pipeline/assemble.mts) reads the
 * records and owns all SBOM construction — graph merge, content-addressed ids. This
 * plugin only RESOLVES and emits raw facts. See src/pipeline/records.mts for the
 * record grammar.
 *
...
object SocketFactsPlugin extends AutoPlugin {
  ...
  override def projectSettings: Seq[Setting[_]] = Seq(
    ...
    socketFacts := {
      ...
      // Scanned configs + raw failures. The abort/warn policy and report rendering live in the TS
      // resolution report, not here.
  ...
  // (IntegrationTest, ...) reach the analysis; source-file-level exclusion belongs to the downstream
  // reachability analysis, not here.
  ...
  // regex pattern sources. src/run/config-glob.mts is the single glob implementation and compiles
  // the caller-facing globs; this plugin only compiles what it receives. A pattern that doesn't
  // compile is dropped, never thrown: the caller emits a dialect-portable subset, so this only
  // guards against a broken transport.
  ...
  // Backslash-escape so a value can never break line/field framing (see src/pipeline/records.mts unescape).
